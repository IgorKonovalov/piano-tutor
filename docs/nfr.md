...
| 7b | **Coach token budget, a whole session.** One `SessionSummary` over a practice session's drills and attempts. | ≤ 6 000 tokens regardless of how many attempts the session took | A `core/` test holds the serialised summary of a session of 40 attempts under the budget, and asserts that a session with ten times the attempts over the same drills stays within a small constant factor — size is a function of drills, not attempts. Measured 2026-09-10: the CLI adds ~1 000 tokens of its own per call on top of whichever summary is sent. |
