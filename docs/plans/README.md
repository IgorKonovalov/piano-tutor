...
| [0010](0010-the-timeline-tells-the-truth-about-the-page.md) | The timeline tells the truth about the page | approved | dev, human | **ADRs 0018 and 0021 together** are its design: 0018 decides what the timeline extracts, 0021 decides who may act on it. Grown on 2026-09-11 from four phases to **nine**, on the user’s ask that the app play the music *as written* — dynamics, hairpins, tempo words, rit./accel., fermata and articulation join the ornaments and pedal it already had. Every mark is read from OSMD’s own interpretation; **we write no interpreter**. It still repairs the two things broken today: a real score that cannot play, and a bar marked wrong for being played *correctly*. **The line it must hold is tempo-freedom** — playback gets every mark, `align.ts` gets none, and Phase 8’s done-when is written as that guard. **Phase 3 renames `grace` to `optional`**, the one identifier Plan 0009 could collide on: not in parallel worktrees without agreeing first. Natural split point if it must be halved is after Phase 4. |
