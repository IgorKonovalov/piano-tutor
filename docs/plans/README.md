...
**Next free number: 0015** (ADRs are a separate sequence; next free there is **0023**.)
...
|------|-------|--------|-------|-----------------|
| [0009](0009-a-bar-is-judged-note-by-note.md) | A bar is judged note by note | approved | dev, human | ADRs 0015, 0016 and 0017 are its design. **Plan 0008's eight followups, taken as one piece of work**, because four of them are the same bug: every reference in the model still draws through the disturbance it is judging around. A bar gains a second number - the shape of its arrivals inside it, read against a quadratic null so a ritardando reads zero - because a hand that hurries three notes and gives them back leaves the bar's duration unchanged to the millisecond. **Its Phase 2 is a `human` phase in the middle of the run** and Plan 0002's last open item rides on it. **Parked 2026-09-10 until the player can sit at the CK88** — it blocks nothing and nothing blocks it. Its Phase 1 is `dev`-only, self-contained and makes no threshold decisions, so it can be picked up alone to have the fixture waiting. |
| [0010](0010-the-timeline-tells-the-truth-about-the-page.md) | The timeline tells the truth about the page | in-progress (Phases 1 and 2 landed; amended 2026-09-11 after Phase 3 stopped on OSMD's ornament realiser, which split it into Phases 3 and 4) | dev, human | **ADRs 0018 and 0021 together** are its design: 0018 decides what the timeline extracts, 0021 decides who may act on it. Grown on 2026-09-11 from four phases to nine, and then to **ten** by the amendment, on the user’s ask that the app play the music *as written* — dynamics, hairpins, tempo words, rit./accel., fermata and articulation join the ornaments and pedal it already had. Every mark is read from OSMD’s own interpretation; **we write no interpreter**. It still repairs the two things broken today: a real score that cannot play, and a bar marked wrong for being played *correctly*. **The line it must hold is tempo-freedom** — playback gets every mark, `align.ts` gets none, and Phase 9’s done-when is written as that guard. **Phase 3 renames `grace` to `optional` and narrows ADR-0009’s subtraction** in `align.ts` and `report.ts`, which is where Plan 0009 could collide: not in parallel worktrees without agreeing first. Natural split point if it must be halved is after Phase 5. |
...
3b. **The timeline tells the truth about the page** — drafted as Plan
   ...
   close, and **widened on 2026-09-11** when the user asked for the app to play the music *as
   written* before moving on to the coach. Nine phases. It still sits first among the plans drafted
   that day because it is the only one repairing things that are broken right now — a real score
   that cannot be played, and a bar marked wrong for being played correctly — but most of it is now
   the expressive layer: dynamics, hairpins, tempo words, rit./accel., fermata, articulation,
   ornaments and pedal, all read from OSMD's own interpretation. ADR-0018 is what it extracts;
   **ADR-0021 is what keeps it away from the aligner**, and that separation is the reason nine
   phases of expression do not cost the tempo-freedom four earlier ADRs bought. If it has to be
   halved, the split is after Phase 4.
